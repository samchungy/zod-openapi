...
> **Note:** To get full TypeScript support for the OpenAPI-specific properties in `.meta()`, you need to import `zod-openapi` somewhere in your project. Importing zod-openapi performs type interface merging with Zod's types, which enables TypeScript to recognize the additional properties (like `id`, `param`, `header`, etc.) in the `.meta()` method. In most cases this will be done where you call `createDocument` or other zod-openapi functions.
