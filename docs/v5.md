...
- Removed: `extendZodWithOpenApi()` and `import zod-openapi/extend`
...
> 💡 **Build-time optimization**: You can now declare `zod-openapi` as a dev dependency if you create schemas at build time.
...
As a result, when a schema with an ID is used in both request and response contexts, the library automatically generates two separate component schemas to properly represent the different behaviors.
...
>
> - `true` — Allow for both input and output
> - `{ input: true, output: true }` — Allow for both input and output
> - `{ input: true }` — Allow for input only
> - `{ output: true }` — Allow for output only
